...
class Run:
    ...
    async def start(self, ctx):
        await self.bot.pool.execute('INSERT INTO hacked ("user", "task") VALUES ($1, $2);', ctx.author.id, 1)
        await ctx.send(self.bot.data.msg)

    @acc()
    @commands.command()
    async def task(self, ctx):
        val = await self.bot.pool.fetchval('SELECT task FROM hacked WHERE "user"=$1;', ctx.author.id)
        try:
    ...
    async def solve(self, ctx, *, code: str):
        ...
        await self.bot.pool.execute('UPDATE hacked SET task=task+1 WHERE "user"=$1;', ctx.author.id)
